...
public class GameService implements IGameService {
	...
	public Game createNewGame(Set<Player> players, List<KillsByPlayer> killsByPlayers, long totalKills) {
		...
		var game = Game.builder().totalKills(totalKills).killsByPlayers(killsByPlayers).players(players).build();
	...
	public Map<String, GameDto> createDtoFromGame(Game game) {
		...
		var playerNames = game.getPlayers().stream().map((p) -> p.getName()).toArray(String[]::new);
		...
		var gameDto = GameDto.builder().players(playerNames).kills(mapPlayersAndKills).totalKills(game.getTotalKills())
				.build();
