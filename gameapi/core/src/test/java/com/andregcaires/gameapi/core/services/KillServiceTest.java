...
public class KillServiceTest {
	...
	public void mustGetTotalAndIndividualPlayerKills() {
		...
		KillsByPlayer objNoWorldKills = killsByPlayer.stream()
				.filter((item) -> item.getPlayer().getName() == "Worf, son of Mogh").findFirst().orElseThrow();

		Assertions.assertEquals(3L, objNoWorldKills.getKills());

		KillsByPlayer objWithWorldKills = killsByPlayer.stream()
				.filter((item) -> item.getPlayer().getName() == "Tasha Yar").findFirst().orElseThrow();
	...
	private void addMockPlayersSet() {
		var player1 = Player.builder().name("Jean-Luc Picard").build();

		var player2 = Player.builder().name("Wesley Crusher").build();

		var player3 = Player.builder().name("Worf, son of Mogh").build();

		var player4 = Player.builder().name("William Riker").build();

		var player5 = Player.builder().name("Tasha Yar").build();

		var player6 = Player.builder().name("Katherine Pulaski").build();

		var player7 = Player.builder().name("Ro Laren").build();

		players.addAll(Arrays.asList(player1, player2, player3, player4, player5, player6, player7));
	}

	private void addMockKillList() {
		var kill1 = Kill.builder().killerPlayer("Jean-Luc Picard").killedPlayer("Wesley Crusher")
				.reason("Shut up, Wesley").build();

		var kill2 = Kill.builder().killerPlayer("Worf, son of Mogh").killedPlayer("William Riker").reason("Bat'leth")
				.build();

		var kill3 = Kill.builder().killerPlayer("Tasha Yar").killedPlayer("Katherine Pulaski")
				.reason("Romulan Disruptor").build();

		var kill4 = Kill.builder().killerPlayer("Worf, son of Mogh").killedPlayer("Ro Laren").reason("Hand Phaser")
				.build();

		var kill5 = Kill.builder().killerPlayer(WORLD).killedPlayer("Tasha Yar").reason("Armus").build();

		var kill6 = Kill.builder().killerPlayer(WORLD).killedPlayer("Tasha Yar").reason("Romulan Empire").build();

		var kill7 = Kill.builder().killerPlayer("Worf, son of Mogh").killedPlayer("Gowron")
				.reason("For the Glory of the Empire").build();
